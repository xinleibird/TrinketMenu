...
function TrinketMenu.BuildMenu()
	...
	for i=0,4 do
		for j=1,GetContainerNumSlots(i) do
			...
			if itemLink then
				...
				if (not wantRelic and equipSlot=="INVTYPE_TRINKET") or 
				   (wantRelic and (equipSlot=="INVTYPE_RELIC" or equipSlot=="INVTYPE_IDOL" or equipSlot=="INVTYPE_LIBRAM" or equipSlot=="INVTYPE_TOTEM")) then
	...
	if TrinketMenu.NumberOfTrinkets<1 then
		...
		for i=1,TrinketMenu.NumberOfTrinkets do
			...
			item:SetPoint("TOPLEFT","TrinketMenu_MenuFrame",TrinketMenu.OverrideMenuDock or TrinketMenuPerOptions.MenuDock,xpos,ypos)
...
function TrinketMenu.Initialize()
	...
	if TrinketMenuPerOptions.Visible=="ON" and (GetInventoryItemLink("player",13) or GetInventoryItemLink("player",14) or (TrinketMenu.HasRelic and GetInventoryItemLink("player",18))) then
...
function TrinketMenu.FindItem(name,includeInventory)
	if includeInventory then
		for i=13,18 do
			if (i==13 or i==14 or (TrinketMenu.HasRelic and i==18)) and string.find(GetInventoryItemLink("player",i) or "",name,1,1) then
				return i
			end
		end
	end
...
function TrinketMenu.OnTrinketMouseEnter()
	...
	if TrinketMenuPerOptions.MainOrient=="HORIZONTAL" then
		...
		TrinketMenu_MenuFrame:SetPoint("BOTTOMLEFT",this,"TOPLEFT",-TrinketMenu.DockInfo("xstart"),40-TrinketMenu.DockInfo("ystart"))
	else
		TrinketMenu.OverrideDock = "TOPRIGHTTOPLEFT" -- Menu TOPLEFT to Anchor TOPRIGHT (Grows Right)
		TrinketMenu.OverrideMenuDock = "TOPLEFT"
		TrinketMenu_MenuFrame:SetPoint("TOPLEFT",this,"TOPRIGHT",TrinketMenu.DockInfo("xoff"),-TrinketMenu.DockInfo("ystart"))
	end
	TrinketMenu.BuildMenu()
