...
TrinketMenuOptions = {
	...
	CooldownCount = "ON", -- whether to display numerical cooldown counters
...
function TrinketMenu.DockInfo(arg1)
	local anchor = TrinketMenu.OverrideDock or (TrinketMenuPerOptions.MainDock .. TrinketMenuPerOptions.MenuDock)
...
function TrinketMenu.DockWindows()
	...
	if TrinketMenuOptions.KeepDocked == "ON" then
		...
		if TrinketMenuOptions.Locked == "OFF" then
			TrinketMenu_MenuFrame:SetPoint(
				TrinketMenuPerOptions.MenuDock,
				"TrinketMenu_MainFrame",
				TrinketMenuPerOptions.MainDock,
				TrinketMenu.DockInfo("xoff"),
				TrinketMenu.DockInfo("yoff")
			)
		else
			TrinketMenu_MenuFrame:SetPoint(
				TrinketMenuPerOptions.MenuDock,
				"TrinketMenu_MainFrame",
				TrinketMenuPerOptions.MainDock,
				TrinketMenu.DockInfo("xoff") * 3,
				TrinketMenu.DockInfo("yoff") * 3
			)
		end
	end
...
function TrinketMenu.OrientWindows()
	TrinketMenu_Trinket0:ClearAllPoints()
	TrinketMenu_Trinket1:ClearAllPoints()
	TrinketMenu_Trinket2:ClearAllPoints()

	if TrinketMenu.HasRelic then
		TrinketMenu_Trinket2:Show()
		if TrinketMenuPerOptions.MainOrient == "HORIZONTAL" then
			TrinketMenu_MainFrame:SetWidth(130)
			TrinketMenu_MainFrame:SetHeight(52)
			TrinketMenu_Trinket0:SetPoint("TOPLEFT", "TrinketMenu_MainFrame", "TOPLEFT", 8, -8)
			TrinketMenu_Trinket1:SetPoint("TOPLEFT", "TrinketMenu_MainFrame", "TOPLEFT", 48, -8)
			TrinketMenu_Trinket2:SetPoint("TOPLEFT", "TrinketMenu_MainFrame", "TOPLEFT", 88, -8)
		else
			TrinketMenu_MainFrame:SetWidth(52)
			TrinketMenu_MainFrame:SetHeight(130)
			TrinketMenu_Trinket0:SetPoint("TOPLEFT", "TrinketMenu_MainFrame", "TOPLEFT", 8, -8)
			TrinketMenu_Trinket1:SetPoint("TOPLEFT", "TrinketMenu_MainFrame", "TOPLEFT", 8, -48)
			TrinketMenu_Trinket2:SetPoint("TOPLEFT", "TrinketMenu_MainFrame", "TOPLEFT", 8, -88)
		end
	else
		TrinketMenu_Trinket2:Hide()
		if TrinketMenuPerOptions.MainOrient == "HORIZONTAL" then
	...
	for i = 0, 4 do
		for j = 1, GetContainerNumSlots(i) do
			...
			if itemLink then
				...
				local slot = TrinketMenu.CurrentMenuSlot or 13
				local wantRelic = (slot == 18)

				if
					(not wantRelic and equipSlot == "INVTYPE_TRINKET")
					or (
						wantRelic
						and (
							equipSlot == "INVTYPE_RELIC"
							or equipSlot == "INVTYPE_IDOL"
							or equipSlot == "INVTYPE_LIBRAM"
							or equipSlot == "INVTYPE_TOTEM"
						)
					)
				then
	...
	if TrinketMenu.NumberOfTrinkets < 1 then
		...
		for i = 1, TrinketMenu.NumberOfTrinkets do
			...
			item:SetPoint(
				...
				TrinketMenu.OverrideMenuDock or TrinketMenuPerOptions.MenuDock,
				xpos,
				ypos
			)
...
function TrinketMenu.Initialize()
	...
	options.CooldownCount = options.CooldownCount or "ON" -- Force ON by default
	...
	if TrinketMenuPerOptions.XPos and TrinketMenuPerOptions.YPos then
		TrinketMenu_MainFrame:SetPoint(
			...
			"UIParent",
			"BOTTOMLEFT",
			TrinketMenuPerOptions.XPos,
			TrinketMenuPerOptions.YPos
		)
	end
	...
	TrinketMenu.CreateTimer("UpdateWornTrinkets", TrinketMenu.UpdateWornTrinkets, 0.75)
	TrinketMenu.CreateTimer("DockingMenu", TrinketMenu.DockingMenu, 0.2, 1)
	TrinketMenu.CreateTimer("MenuMouseover", TrinketMenu.MenuMouseover, 0.25, 1)
	TrinketMenu.CreateTimer("Scaling", TrinketMenu.Scaling, 0.1, 1)
	...
	TrinketMenu.HasRelic = (class == "PALADIN" or class == "SHAMAN" or class == "DRUID")
	...
	if
		...
		and (
			...
			or GetInventoryItemLink("player", 14)
			or (TrinketMenu.HasRelic and GetInventoryItemLink("player", 18))
		)
	then
...
function TrinketMenu.ItemInfo(slot)
	...
	if link then
		...
		if slot == 18 then
			_, texture = GetInventorySlotInfo("RangedSlot")
		else
...
function TrinketMenu.FindItem(name, includeInventory)
	if includeInventory then
		for i = 13, 18 do
			if
				(i == 13 or i == 14 or (TrinketMenu.HasRelic and i == 18))
				and string.find(GetInventoryItemLink("player", i) or "", name, 1, 1)
			then
				return i
			end
		end
...
function TrinketMenu.OnEvent()
	...
	elseif
		(event == "PLAYER_REGEN_ENABLED" or event == "PLAYER_UNGHOST" or event == "PLAYER_ALIVE")
		and not TrinketMenu.IsPlayerReallyDead()
	then
...
function TrinketMenu.UpdateWornTrinkets()
	...
	if TrinketMenu.HasRelic then
		TrinketMenu_Trinket2Icon:SetTexture(TrinketMenu.ItemInfo(18))
		TrinketMenu_Trinket2Icon:SetDesaturated(0)
		TrinketMenu_Trinket2:SetChecked(0)
	end
	...
	local name
	for i = 13, 18 do
		if i == 13 or i == 14 or (TrinketMenu.HasRelic and i == 18) then
...
function TrinketMenu.TimerDebug()
	...
	DEFAULT_CHAT_FRAME:AddMessage(
		"|cFF44AAFFTrinketMenu_TimersFrame is " .. (TrinketMenu_TimersFrame:IsVisible() and on or off)
	)
	...
	if IsShiftKeyDown() and ChatFrameEditBox:IsVisible() then
		ChatFrameEditBox:Insert(
			GetContainerItemLink(
				TrinketMenu.BaggedTrinkets[this:GetID()].bag,
				TrinketMenu.BaggedTrinkets[this:GetID()].slot
			)
		)
	else
		local slot = (TrinketMenu.CurrentMenuSlot == 18) and 18 or (arg1 == "LeftButton") and 13 or 14
		if TrinketMenu.QueueInit and slot ~= 18 then
			local _, _, canCooldown = GetContainerItemCooldown(
				TrinketMenu.BaggedTrinkets[this:GetID()].bag,
				TrinketMenu.BaggedTrinkets[this:GetID()].slot
			)
...
function TrinketMenu.MenuMouseover()
	if
		(not MouseIsOver(TrinketMenu_MainFrame))
		and (not MouseIsOver(TrinketMenu_MenuFrame))
		and not IsShiftKeyDown()
		and (TrinketMenuOptions.KeepOpen == "OFF")
		and not TrinketMenu.IsTimerActive("Scaling")
	then
		TrinketMenu.StopTimer("MenuMouseover")
		TrinketMenu_MenuFrame:Hide()
		TrinketMenu_MenuFrame:Hide()
		TrinketMenu.OverrideDock = nil -- clear override when menu hides
		TrinketMenu.OverrideMenuDock = nil
	end
end
...
function TrinketMenu.OnTrinketMouseEnter()
	...
	TrinketMenu_MenuFrame:ClearAllPoints()
	if TrinketMenuPerOptions.MainOrient == "HORIZONTAL" then
		TrinketMenu.OverrideDock = "TOPLEFTBOTTOMLEFT" -- Menu BOTTOMLEFT to Anchor TOPLEFT (Grows Up)
		TrinketMenu.OverrideMenuDock = "BOTTOMLEFT"
		TrinketMenu_MenuFrame:SetPoint(
			"BOTTOMLEFT",
			this,
			"TOPLEFT",
			-TrinketMenu.DockInfo("xstart"),
			40 - TrinketMenu.DockInfo("ystart")
		)
	else
		TrinketMenu.OverrideDock = "TOPRIGHTTOPLEFT" -- Menu TOPLEFT to Anchor TOPRIGHT (Grows Right)
		TrinketMenu.OverrideMenuDock = "TOPLEFT"
		TrinketMenu_MenuFrame:SetPoint(
			"TOPLEFT",
			this,
			"TOPRIGHT",
			TrinketMenu.DockInfo("xoff"),
			-TrinketMenu.DockInfo("ystart")
		)
	end
	TrinketMenu.BuildMenu()
...
function TrinketMenu.UpdateWornCooldowns(maybeGlobal)
	...
	if TrinketMenu.HasRelic then
		start, duration, enable = GetInventoryItemCooldown("player", 18)
		CooldownFrame_SetTimer(TrinketMenu_Trinket2Cooldown, start, duration, enable)
	end
...
function TrinketMenu.UpdateMenuCooldowns()
	...
	for i = 1, TrinketMenu.NumberOfTrinkets do
		start, duration, enable =
			GetContainerItemCooldown(TrinketMenu.BaggedTrinkets[i].bag, TrinketMenu.BaggedTrinkets[i].slot)
...
function TrinketMenu.ReflectTrinketUse(slot)
	...
	elseif slot == 18 then
		TrinketMenu_Trinket2:SetChecked(1)
	end
...
function TrinketMenu.newUseInventoryItem(slot)
	...
	if (slot == 13 or slot == 14 or slot == 18) and not MerchantFrame:IsVisible() then
...
function TrinketMenu.newUseAction(slot, cursor, self)
	...
	if IsEquippedAction(slot) then
		...
		local _, trinket2 = TrinketMenu.ItemInfo(18)
		...
		elseif GameTooltipTextLeft1:GetText() == trinket2 then
			TrinketMenu.ReflectTrinketUse(18)
		end
	end
end
	...
	if TrinketMenuOptions.TinyTooltips == "ON" then
		...
		for i = 2, 12 do
			...
			if line:IsVisible() then
				...
				if
					string.find(line, TrinketMenu.ITEM_SPELL_CHARGES_P1)
					or string.find(line, TrinketMenu.ITEM_SPELL_CHARGES)
				then
					charge = line
				end
			end
...
function TrinketMenu.EquipTrinketByName(name, slot)
	...
	if (UnitAffectingCombat("player") or TrinketMenu.IsPlayerReallyDead()) and slot ~= 18 then
		-- queue trinket
	...
	elseif not CursorHasItem() and not SpellIsTargeting() then
		...
		if b then
			...
			if not isLocked and not IsInventoryItemLocked(slot) then
				...
				local iconSlot = (slot == 18) and 2 or (slot - 13)
				getglobal("TrinketMenu_Trinket" .. iconSlot .. "Icon"):SetDesaturated(1)
...
function TrinketMenu.AddWatchItem(name, inv, bag, slot)
	...
	if not inv and not bag then
		...
		if
			not string.find(
				(inv and (GetInventoryItemLink("player", inv) or ""))
					or (bag and (GetContainerItemLink(bag, slot) or ""))
					or "",
				name
			)
		then
...
function TrinketMenu.WriteWornCooldowns()
	...
	if TrinketMenu.HasRelic then
		start, duration = GetInventoryItemCooldown("player", 18)
		TrinketMenu.WriteCooldown(TrinketMenu_Trinket2Time, start, duration)
	end
end
...
function TrinketMenu.WriteCooldown(where, start, duration)
	...
	elseif cooldown < 3 and not where:GetText() then
		...
		where:SetText(
			(cooldown < 60 and math.floor(cooldown + 0.5) .. " s")
